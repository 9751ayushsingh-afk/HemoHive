...
export async function POST(request: Request) {
    ...
    try {
        ...
        // 1. Find Search-Expired Deliveries
        ...
        console.log(`[CLEANUP] Found ${expiredDeliveries.length} expired delivery proposals. Triggering auto-reject...`);
        ...
        const results = await Promise.all(expiredDeliveries.map(async (delivery) => {
            ...
            try {
                const res = await fetch(`${protocol}://${host}/api/delivery/request`, {
                    ...
                    body: JSON.stringify({ requestId: delivery.requestId })
                });
