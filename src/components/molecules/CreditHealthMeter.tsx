...
interface CreditHealthMeterProps {
  value: number;
}

const CreditHealthMeter: React.FC<CreditHealthMeterProps> = ({ value }) => {
  const liquidRef = useRef<SVGGElement>(null);
  const textRef = useRef<SVGTextElement>(null);
  const bubblesRef = useRef<(SVGCircleElement | null)[]>([]);
  ...
  useEffect(() => {
    ...
    gsap.to(animatedValue, {
      ...
      ease: 'power3.out',
      onUpdate: () => {
        if (text) text.textContent = `${Math.round(animatedValue.val)}%`;
      }
    });

    const liquidPath = liquid.querySelector('path');
    if (liquidPath) {
      gsap.to(liquidPath, { fill: `url(#liquid-gradient-${value})`, duration: 1 });
    ...
    // Cleanup function not strictly necessary for simple gsap calls but good practice
    return () => {
      gsap.killTweensOf([liquid, text, circle, ...bubblesRef.current]);
    };
